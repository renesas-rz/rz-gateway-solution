...
THING_GROUP_NAME=$(get_config "iot.thing_group_name")
GREENGRASS_VERSION=$(get_config "greengrass.version")
ROLE_NAME=$(get_config "iot.role_name")
ROLE_ACCESS=$(get_config "iot.role_access")
ROLE_ALIAS=$(get_config "iot.role_alias")
CERTS_DIR=$(get_config "aws.certs_dir")
POLICY_NAME=$(get_config "iot.policy_name")
...
install_or_verify_aws_cli() {
    if command -v aws &>/dev/null; then
        ...
        if sudo apt-get install -y awscli >>"$LOG_FILE" 2>&1; then
            log "AWS CLI installed via apt."
        else
            log "Fallback: Installing AWS CLI via pip3..."
            pip3 install --user awscli >>"$LOG_FILE" 2>&1 || {
    ...
    if ! aws --version >>"$LOG_FILE" 2>&1; then
        log "ERROR: AWS CLI installation verification failed."
        exit 1
    fi
}
...
create_iot_policy() {
    ...
    # Check if policy exists
    if aws iot get-policy --policy-name "$POLICY_NAME" --region "$AWS_REGION" &>/dev/null; then
        log " IoT policy already exists: $POLICY_NAME — deleting and recreating..."

        # Find attached certificates and detach them
        ...
        for CERT_ARN in $CERT_ARNS; do
            log " Detaching policy from certificate: $CERT_ARN"
        ...
        aws iot delete-policy \
            --policy-name "$POLICY_NAME" \
            --region "$AWS_REGION" || {
            log " ERROR: Failed to delete existing IoT policy."
            exit 1
        }
    fi

    # Create IoT policy file with shadow permissions
    cat > /tmp/iot-policy.json <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
...
EOF
    ...
    # Re-attach policy to the certificate (if CERT_ARN was known)
    if [ -n "$CERT_ARNS" ]; then
        for CERT_ARN in $CERT_ARNS; do
            log " Reattaching policy to certificate: $CERT_ARN"
...
EOF
    ...
    if ! aws iot describe-role-alias --role-alias "$ROLE_ALIAS" --region "$AWS_REGION" &>/dev/null; then
        aws iot create-role-alias --role-alias "$ROLE_ALIAS" --role-arn "$ROLE_ARN" --region "$AWS_REGION" || {
...
deploy_nucleus_12_0_0() {
    sleep 10
    target_arn=`aws iot describe-thing --thing-name $THING_NAME --region "$AWS_REGION" | grep thingArn`
...
configure_greengrass_core() {
    ...
    sudo -E aws iot create-keys-and-certificate \
        ...
        --region "$AWS_REGION" | sudo tee cert.json >/dev/null
    ...
    aws iot attach-policy --policy-name "$POLICY_NAME" --target "$CERT_ARN" --region "$AWS_REGION"

    # Attach the certificate to the IoT Thing principal
    aws iot attach-thing-principal --thing-name "$THING_NAME" --principal "$CERT_ARN" --region "$AWS_REGION"
...
create_iot_policy
create_iam_role
# create_systemd_service # should be commmented
# start_greengrass_core  #should be commented.
verify_greengrass_status
print_versions


log "Home automation Client setup complete with Greengrass Core"
