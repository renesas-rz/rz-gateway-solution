...
LAMBDA_ZIP_SOURCE="../charging_point_greengrass.zip"
ANSIBLE_DIR="ansible_start"
CREDENTIALS_FILE="$ANSIBLE_DIR/credentials.txt"
PLAYBOOK_DEST="$ANSIBLE_DIR/playbook.yaml"
LAMBDA_DEST="$ANSIBLE_DIR/charging_point_greengrass.zip"
SRC_BACKEND_DIR="$(realpath ../src/backend)"
SRC_FRONTEND_BUILD_DIR="$(realpath ../src/evse_frontend/build)"
SERVICE_FILE="$SRC_BACKEND_DIR/systemd-ocpp.service"
...
install_base_packages() {
    log "Installing Ansible + Python AWS packages..."
    ...
    # Install packages with --user flag for security
    pip3 install --user --quiet boto boto3 ansible >>"$LOG_FILE" 2>&1 || {
    ...
    ansible-galaxy collection install amazon.aws >>"$LOG_FILE" 2>&1 || {
        log "ERROR: Failed to install Ansible AWS collection."
        exit 1
    }
...
setup_ansible() {
    ...
    # local key_path="$HOME/.ssh/ocpp-server-key.pem"
     local key_path="$(pwd)/ocpp-server-key.pem"
    
if [[ ! -f "$key_path" ]]; then
    # Local key file doesn't exist, check AWS

    if aws ec2 describe-key-pairs --key-names ocpp-server-key --region "$REGION" &>/dev/null; then
        log "Key pair 'ocpp-server-key' already exists in AWS. Deleting it to create a new one..."
        aws ec2 delete-key-pair --key-name ocpp-server-key --region "$REGION" || {
...
get_ip_address() {  
    ...
    while [[ $attempt -le $max_attempts ]]; do
        ...
        ip_addr=$(aws ec2 --region "$REGION" \
            describe-instances \
            --filters "Name=tag:Name,Values=ocpp-server" \
...
# === MAIN EXECUTION ===
main() {
    log "Starting OCPP Server Launch Setup..."
    ...
    log "Copying lambda ocpp zip file from $LAMBDA_ZIP_SOURCE to $LAMBDA_DEST..."
    cp "$PLAYBOOK_SOURCE" "$PLAYBOOK_DEST" || {
    log "ERROR: Failed to copy lambda zip to ansible directory."
    exit 1
    }
